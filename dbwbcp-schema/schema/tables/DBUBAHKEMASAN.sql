...
SELECT NoBukti, SUM(SubTotal) AS TotSubTotal, SUM(NDiskon) AS TotDiskon,
       ...
       SUM(NPPN) AS TotPPN, SUM(NNet) AS TotNet, SUM(SubTotalRp) AS TotSubTotalRp,
       SUM(NDiskon * Kurs) AS TotDiskonRp, SUM(SubTotalRp) - SUM(NDiskon * Kurs) AS TotTotalRp,
       SUM(NDPPRp) AS TotDPPRp, SUM(NPPNRp) AS TotPPNRp, SUM(NNETRp) AS TotNetRp
FROM dbRBeliDet
GROUP BY NoBukti
...
SELECT A.NOBUKTI, A.TANGGAL, B.DEVISI, A.NOTE, A.LAMPIRAN,
       ...
       ISOTORISASI5, OTOUSER5, TGLOTO5, B.URUT, B.PERKIRAAN, B.LAWAN,
       B.KETERANGAN, B.KETERANGAN2, B.DEBET, B.KREDIT, B.VALAS, B.KURS,
       B.DEBETRP, B.KREDITRP, B.STATUSGIRO, B.MYID, 'KASBANK' AS JENIS,
       A.NOURUT
FROM DBO.DBTRANS A
...
( NAME = N'DBSuryaZigZag_log1', FILENAME = N'd:\MSSQL\dbwbcp_2.ldf', SIZE = 82816KB, MAXSIZE = 2048GB, FILEGROWTH = 10%)
GO
ALTER DATABASE [dbwbcp] SET COMPATIBILITY_LEVEL = 100
GO
IF (1 = FULLTEXTSERVICEPROPERTY('IsFullTextInstalled'))
BEGIN
    EXEC [dbwbcp].[dbo].[sp_fulltext_database] @action = 'enable'
END
GO
ALTER DATABASE [dbwbcp] SET ANSI_NULL_DEFAULT OFF
GO
ALTER DATABASE [dbwbcp] SET ANSI_NULLS OFF
GO
ALTER DATABASE [dbwbcp] SET ANSI_PADDING OFF
GO
ALTER DATABASE [dbwbcp] SET ANSI_WARNINGS OFF
GO
ALTER DATABASE [dbwbcp] SET ARITHABORT OFF
GO
ALTER DATABASE [dbwbcp] SET AUTO_CLOSE OFF
GO
ALTER DATABASE [dbwbcp] SET AUTO_CREATE_STATISTICS ON
GO
ALTER DATABASE [dbwbcp] SET AUTO_SHRINK ON
GO
ALTER DATABASE [dbwbcp] SET AUTO_UPDATE_STATISTICS ON
GO
ALTER DATABASE [dbwbcp] SET CURSOR_CLOSE_ON_COMMIT OFF
GO
ALTER DATABASE [dbwbcp] SET CURSOR_DEFAULT GLOBAL
GO
ALTER DATABASE [dbwbcp] SET CONCAT_NULL_YIELDS_NULL OFF
GO
ALTER DATABASE [dbwbcp] SET NUMERIC_ROUNDABORT OFF
GO
ALTER DATABASE [dbwbcp] SET QUOTED_IDENTIFIER OFF
GO
ALTER DATABASE [dbwbcp] SET RECURSIVE_TRIGGERS OFF
GO
ALTER DATABASE [dbwbcp] SET DISABLE_BROKER
GO
ALTER DATABASE [dbwbcp] SET AUTO_UPDATE_STATISTICS_ASYNC ON
GO
ALTER DATABASE [dbwbcp] SET DATE_CORRELATION_OPTIMIZATION OFF
GO
ALTER DATABASE [dbwbcp] SET TRUSTWORTHY OFF
GO
ALTER DATABASE [dbwbcp] SET ALLOW_SNAPSHOT_ISOLATION OFF
GO
ALTER DATABASE [dbwbcp] SET PARAMETERIZATION SIMPLE
GO
ALTER DATABASE [dbwbcp] SET READ_COMMITTED_SNAPSHOT OFF
GO
ALTER DATABASE [dbwbcp] SET HONOR_BROKER_PRIORITY OFF
GO
ALTER DATABASE [dbwbcp] SET READ_WRITE
GO
...
GO
EXEC sys.sp_db_vardecimal_storage_format N'dbwbcp', N'ON'
GO

README.md:
# Project Documentation

This project contains the database schema for the application. It includes tables, views, users, and roles necessary for the application's functionality. Each SQL file defines a specific part of the schema, ensuring modularity and ease of maintenance.